...
Valtab = Data("table_of_values.csv")
...
def testing_Hp():
    ...
    line1, = ax1.plot(xvals, ratioddHp, label=r"$\frac{1}{\mathcal{H}}\frac{\mathrm{d}^2\mathcal{H}}{\mathrm{d}x^2}$", color=colors[0])
    line2, = ax1.plot(xvals, ratiodHp, label=r"$\frac{1}{\mathcal{H}}\frac{\mathrm{d}\mathcal{H}}{\mathrm{d}x}$", color=colors[-1])

    rad_area = ax1.axvspan(x_min, x_RM-tol, color="orange", alpha=.1, label=r"$\Omega_\mathrm{rad}$")
    mat_area = ax1.axvspan(x_RM+tol, x_ML-tol, color="green", alpha=.1, label=r"$\Omega_\mathrm{M}$")
    lam_area = ax1.axvspan(x_ML+tol, x_max, color="purple", alpha=.1, label=r"$\Omega_\Lambda$")
    ...
    legend2 = HpFig.legend([rad_area, mat_area, lam_area], [rad_area.get_label(), mat_area.get_label(), lam_area.get_label()], loc="upper right", fancybox=True, ncols=3, bbox_to_anchor=[0.97,0.965], fontsize=24)
...
def testing_eta():
    ...
    line1, = ax1.plot(xvals, etaHpc, label=r"$\frac{\eta\mathcal{H}}{c}$", color=colors[0])
    line2, = ax1.plot(xvals, detaHpc, label=r"$\frac{\mathcal{H}}{c}\frac{\mathrm{d}\eta}{\mathrm{d}x}$", color=colors[-1])
    ...
    legend2 = etaFig.legend([rad_area, mat_area, lam_area], [rad_area.get_label(), mat_area.get_label(), lam_area.get_label()], loc="upper right", fancybox=True, ncols=3, bbox_to_anchor=[0.97,0.965], fontsize=24)
