...
\subsubsection{Metric perturbations}
    ...
    \begin{subequations}\label{eq:m3:theory:geodesic_results}
        \begin{align}
            ...
            \dv{p^i}{t} &= -\left(H+\dv{\Phi}{t}\right)p^i-\frac{E}{a}\Phi_i-\frac{1}{a}\frac{p^i}{E}p^k\Phi_k + \frac{p^2}{aE}\Phi_i \label{eq:m3:theory:metric_perturb_dpidt}\\
            \dv{p}{t} &= -\left(H+\dv{\Phi}{t}\right)p-\frac{E}{a}\hat{p}^i\Psi_i \label{eq:m3:theory:metric_perturb_dpdt}
        \end{align}
    \end{subequations}
...
\subsubsection{Einstein-Boltzmann equations}
    ...
    \begin{equation}\label{eq:m3:theory:boltzmann_for_radiation}
        \dv{f}{t} = \pdv{f}{t}+\frac{\hat{p}^i}{a}\pdv{f}{x^i}-\left[H+\dv{\Phi}{t} + \frac{1}{a}\hat{p}^i\pdv{\Psi}{x^i}\right]p\pdv{f}{p}.
    \end{equation}
