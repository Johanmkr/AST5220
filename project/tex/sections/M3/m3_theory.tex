...
\subsubsection{Fourier space and multipole expansion}
    ...
    \begin{equation}\label{eq:m3:theory:fourier_pde_tricks}
        ...
        \end{split}
    \end{equation} 
    The two equations in ~\cref{eq:m3:theory:fourier_pde_tricks} have the ability of reducing partial differential equations (PDEs) down to a set of decoupled ordinary differential equations (ODEs). This means that we are able to solve for each mode $k=\abs{\vec{k}}$ independently, which will be of great impact for the equations to come. 
...
\subsubsection{Modes, scales and the horizon}\label{sec:m3:theory:modes_scales_horizon}
    ...
    \begin{equation}\label{eq:m3:theory:mode_scale_definition}
        ...
        \end{split}
    \end{equation}
    The horizon will evolve in time, and we expect these modes at different scales to also evolve differently in time. At very early times, the horizon is very small, and we expect add modes to be larger than this. We define the concept of \textit{super-horizon} modes, which are modes whose scale is larger than the horizon. We ultimately also have \textit{sub-horzon} modes, where the scale is smaller than the horizon. Different modes are expected to undergo \textit{horizon crossing} at different times. 
